...
(function () {      //Main function Begin
    ...
    var wireEvents = function () {  //wireEvents Start
        ...
        $(document).on("click", "#testDialog", function () {   //To test dialog box
            $('#dialogDiv').dialog("close");
        }); 

         $(document).on("click", "#createSpan", function () {   //To close Label saved success
            refreshProds("");
        }); //End
        ...
        $(document).on("click", "#btnUpload", function () {    //To upload a selected image
            ...
            ProductServer.uploadProductImage(product);
//            var newImageName = "../images/Product/"+test+"_Product";
//            $("#"+test).attr("src",newImageName);11
            $('#msgDiv').css("display", "inline");
            $('#msgLbl').html("Image Successfully Updated <span id='createSpan' style='color:white' class='glyphicon glyphicon-remove'></span>").css("display", "inline");
            $('#modalHeader').html("Image Updated");
            $('#modalBody').html("Image Updated Successfully");
        ...
        $(document).on("click", "#removeBtn", function () {      //To open remove dialogue
            ...
            global_id = getId(this);
                ...
                $(document).on("click", "#modalFooter", function () {      //To open created Modal dialogue
                refreshProds();
        }); //End
        ...
        $(document).on("click", "#modalRemoveBtn", function () {  //To remove product from list
            var id = global_id;
            var product = { "_id": id };
            ProductServer.removeProduct(product);
            $('#myModal').modal('hide');
             $('#msgDiv').css("display", "inline");
            $('#msgLbl').html("Product Successfully Removed <span id='createSpan' style='color:white' class='glyphicon glyphicon-remove'></span>").css("display", "inline");
            $('#modalHeader').html("Product Removed");
            $('#modalBody').html("Product Removed Successfully");
            $('#createModal').modal('show');
            return false;
        }); //End
        ...
        $(document).on("click", "#submitBtn", function () {    //To submit form to create product 
            ...
            $('#formValidate').html("Please Fill all Fields").css("display", "inline");
                return false;
            }
            //Form validation end
            ...
            ProductServer.createProduct(product); //create API call
            //$('#formValidate').html("Product Successfully Saved").css({"display":"inline","background-color":"green"});
                $('#formValidate').html("Product Successfully Saved <span id='createSpan' style='color:white' class='glyphicon glyphicon-remove'></span>").css({ "display": "inline", "background-color": "green","font-size":"small" });
            $('#modalHeader').html("Product Created");
            $('#modalBody').html(product.name+" Created Successfully");
            $('#createModal').modal('show');
            return false;
        }); //End
        ...
        $(document).on("click", "#submitEditBtn", function () {     //To submit form to edited product
            ...
            //Form validation begin
            if (name == '' || desc == '' || ctg == '' || (price == '' && price != 0)) {
            $('#editformValidate').html("Please Fill all Fields").css("display", "inline");
                return false;
            }
            //Form validation end
            var product = {
            ...
            ProductServer.updateProduct(product); //Edit API call
            $('#editformValidate').html("Product Successfully Updated <span id='createSpan' style='color:white' class='glyphicon glyphicon-remove'></span>").css({ "display": "inline", "background-color": "green","font-size":"small" });
            $('#modalHeader').html("Product Updated");
            $('#modalBody').html(product.name+" Updated Successfully");
            $('#createModal').modal('show');
            return false;
        }); //End
